document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Read the site prefix from the data attribute on the grid itself.
    const sitePrefix = grid.dataset.sitePrefix || '';

    // SVG icon for the PDF cards
    const pdfIcon = `
        <svg xmlns="http://www.w.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="icon">
    ...
    if (grid && typeof libraryData !== 'undefined') {
        ...
        libraryData.forEach(paper => {
            ...
            // 2. Use the prefix to construct the correct URL.
            // We ensure no double slashes if the prefix is just '/'
            const finalUrl = (sitePrefix.endsWith('/') ? sitePrefix.slice(0, -1) : sitePrefix) + paper.url;
            card.href = finalUrl;
